...
CIIE = bool(int(sys.argv[12]))
max_growths = int(sys.argv[13])
n_sim = int(sys.argv[14])
...
args = (muN, muP, n_cassettes, n_int_cassettes, kdiff, kMB, kMD, kMI, K, kPL,V_act,Vmax,Km,MIC,abx_in,
        selection, t_max, dt, summary_cols, n_sim, max_growths, plotting,fixed_end,complete_growths,CIIE)

res_list = run_diff_stoch_MP_batch(args)
...
if CIIE:
    fname = f'{date}_{circuit}_{burden}burden_{int(K)}K_{n_cassettes}cassettes_{n_int_cassettes}int_{int(kdiff*100)}kdiff_{Vmax_x}xabxdeg_{int(abx_in)}abx_{kPLexp}kPL_{selection}_CIIE.pkl'
else:
     fname = f'{date}_{circuit}_{burden}burden_{int(K)}K_{n_cassettes}cassettes_{n_int_cassettes}int_{int(kdiff*100)}kdiff_{Vmax_x}xabxdeg_{int(abx_in)}abx_{kPLexp}kPL_{selection}.pkl'
