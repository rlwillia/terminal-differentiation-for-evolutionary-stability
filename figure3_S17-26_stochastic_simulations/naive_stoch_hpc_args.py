...
K = int(sys.argv[4])
Vmax_x = int(sys.argv[5])
abx_in = float(sys.argv[6])
kPL = float(sys.argv[7])
selection = sys.argv[8]
muP_single_cassette = bool(int(sys.argv[9]))
max_growths = int(sys.argv[10])
n_sim = int(sys.argv[11])
...
args = (muN, muP, n_cassettes, selection, kMBs, K, kPL,V_act,Vmax,Km,MIC,abx_in,t_max, dt, 
        summary_cols, n_sim, max_growths, plotting,fixed_end,complete_growths, muP_single_cassette)

res_list = run_naive_stoch_MP_batch(args)
...
fname = f'{date}_{circuit}_{selection}_{burden}burden_{int(K)}K_{Vmax_x}xabxdeg_{int(abx_in)}abx_{kPLexp}kPL_{n_cassettes}cassettes_MPSC_{muP_single_cassette}.pkl'
